...
export default function NettoyageAdminPage() {
  const [services, setServices] = useState<Service[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [modalOpen, setModalOpen] = useState(false);
  const [form, setForm] = useState({ subcategory: "homme", name: "", price: "" });
  const router = useRouter();

  const fetchData = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await fetch("http://localhost:5001/api/services?category=nettoyage", {
  ...
  const handleSubmit = async () => {
    ...
    await fetch("http://localhost:5001/api/admin/services", {
      ...
      body: JSON.stringify({ category: "nettoyage", ...form, price: Number(form.price) }),
    });
    setModalOpen(false);
    setForm({ subcategory: "homme", name: "", price: "" });
    fetchData();
  };
      ...
      {loading ? (
        ...
        <table className="min-w-full divide-y divide-white/10 bg-black/30 backdrop-blur-lg rounded-2xl border border-white/10 shadow-lg">
          <thead className="bg-white/10">
            <tr>
              <th className="px-4 py-2 text-left text-sm">Sous-catégorie</th>
          ...
          <tbody className="divide-y divide-white/10">
            {services.map((s) => (
              <tr key={s._id}>
                <td className="px-4 py-2 text-gray-300">{s.subcategory}</td>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-gray-800 p-6 rounded-xl w-80">
            ...
            >
              {['homme', 'femme', 'enfants', 'linge', 'traditionnels'].map((v) => (
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => setModalOpen(false)} className="text-gray-400">Annuler</button>
              <button onClick={handleSubmit} className="bg-pink-600 px-4 py-1 rounded">Enregistrer</button>
            </div>
          </div>
        </div>
      )}
