...
export default function PressingAdminPage() {
  const [services, setServices] = useState<Service[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [modalOpen, setModalOpen] = useState(false);
  const [form, setForm] = useState({ subcategory: "costume", name: "", price: "" });
  const router = useRouter();

  const fetchData = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await fetch("http://localhost:5001/api/services?category=pressing", {
  ...
  const handleSubmit = async () => {
    ...
    await fetch("http://localhost:5001/api/admin/services", {
      ...
      body: JSON.stringify({ category: "pressing", ...form, price: Number(form.price) }),
    });
    setModalOpen(false);
    setForm({ subcategory: "costume", name: "", price: "" });
    fetchData();
  };
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-gray-800 p-6 rounded-xl w-80">
            ...
            >
              {['costume', 'veste', 'chemise', 'pantalon'].map((v) => (
