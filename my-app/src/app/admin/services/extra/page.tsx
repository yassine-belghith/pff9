...
export default function ExtraAdminPage() {
  ...
  const subcategories = ["extra"];

  const fetchData = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await fetch("http://localhost:5001/api/services?category=extra", {
  ...
  const handleSubmit = async () => {
    ...
    setModalOpen(false);
    setForm({ subcategory: "extra", name: "", price: "" });
    fetchData();
  };
