...
export default function TapisAdminPage() {
  const [services, setServices] = useState<Service[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [modalOpen, setModalOpen] = useState(false);
  const [form, setForm] = useState({ 
    subcategory: "tapis_synthetique", 
    name: "", 
    price: "" 
  });
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch("http://localhost:5001/api/services?category=tapis", {
  ...
  const handleSubmit = async () => {
    ...
    setModalOpen(false);
    setForm({ subcategory: "tapis_synthetique", name: "", price: "" });
    fetchData();
  };
            ...
            {services.map((s) => (
              <tr key={s._id}>
                <td className="px-4 py-2 text-gray-300">
                  {s.subcategory.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}
                </td>
